...
func (sh *StockHeader) Validate() error {
	err := validation.ValidateStruct(sh,
		...
		validation.Field(&sh.Custom, validation.When(sh.StockType == global.In, validation.Required.Error("供应商不能为空"))),
		validation.Field(&sh.Courier, validation.When(sh.StockType != global.In, validation.Required.Error("客户不能为空"))),
	)
	return err
}

func (sh *StockHeader) StockHeaderLog(sb []StockBody) (m error, success bool) {
